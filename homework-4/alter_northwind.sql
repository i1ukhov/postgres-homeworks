...
-- Удаляем связанные записи из таблицы order_details, где продукт, который снят с продажи, есть в product_id заказа
DELETE FROM order_details
WHERE EXISTS (SELECT * FROM products WHERE discontinued = 1 AND order_details.product_id = products.product_id);
-- Удаляем непосредственно продукты, снятые с продажи, в таблице products
